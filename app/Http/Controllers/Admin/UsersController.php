...
class UsersController extends JoshController
{
    ...
    public function data()
    {
        ...
        return DataTables::of($users)
            ...
            ->addColumn('actions',function($user) {
                ...
                if ((Sentinel::getUser()->id != $user->id) && ($user->id != 1) && ($user->id != 2)) {
    ...
    public function edit(User $user)
    {
        ...
        $countries = $this->countries;
//        $role1 = Sentinel::findRoleById(1);
//        $admin=$role1->users()->with('roles')->get();
//        $admin_count=$admin->count();
//        $admin_role_count=Sentinel::findUserById($user->id)->getRoles()->first()->id;
        $role = Sentinel::findRoleById(2);
        $users = $role->users()->with('roles')->get();
        $users_count = $users->count();
        $user_role_id = Sentinel::findUserById($user->id)->getRoles()->first()->id;

        // Show the page
        return view('admin.users.edit', compact('user', 'roles', 'userRoles', 'countries', 'status','users_count','user_role_id'));
    }
    ...
    public function update(User $user, UserRequest $request)
    {
        ...
        // Redirect to the user page
//        return back()->with('success', trans('forgot-password.success'));
